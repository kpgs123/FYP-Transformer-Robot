...
if not finished:
    ...
    new_img = np.array(en)
    turning_cord = (float('inf'), float('inf'))
    ...
    for j in range(coll_cord[0] + 150, coll_cord[0] - 150, -15):
        for k in range(coll_cord[1] + 150, coll_cord[1] - 150, -15):
            ...
            if not is_obstcle_inside_the_shape_o(y_set[0], y_set[1], x_set[0], x_set[1], maze, 0):
                if abs(j-coll_cord[0]) + abs(k-coll_cord[1]) < abs(turning_cord[0] - coll_cord[0]) + abs(turning_cord[1] - coll_cord[1]):
                    x_px_t = x_px[:]
                    y_px_t = y_px[:]
                    turning_cord = (j, k)
    ...
    # Draw the square on the image
    points = np.array([(x, y) for x, y in zip(x_px_t, y_px_t)], np.int32)
    print(x_px_t, y_px_t)
    points = points.reshape((-1, 1, 2))
    color = (255, 0, 0)  # Blue color in BGR format
    thickness = 1
    ...
    final_path['O'] = final_path['O'][:final_path['O'].index(min_key)+1]

    path_length2, path2 = astar(min_key, turning_cord, maze, prox_maze)

    final_path['O'].extend(path2[3:])

    x_cm = [8.5, 8.5, -25.5, -25.5, 8.5]
    y_cm = [8.5, -25.5, -25.5, 8.5, 8.5]

    for i in range(3, len(path2)-1):
        y1, x1 = path2[i]
        y2, x2 = path2[i+1]
        cv.line(new_img, (x1, y1), (x2, y2), (0, 255, 0), 1)
